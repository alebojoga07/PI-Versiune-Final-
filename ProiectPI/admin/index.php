...
$orderid = $_POST['order_id'];
    $query = "UPDATE `orders` SET `status` = 'expediat' WHERE `orders`.`order_id` = $orderid";
        ...
        <header class="bg-dark py-5">
            <div class="container px-4 px-lg-5 my-5">
                <div class="text-center text-white">
                    <h3 class="display-4 fw-bolder">Panou Admin - Comenzi</h3>
                </div>
            </div>
        </header>
        <!-- Section-->
...
?>
<? if ($product['status'] !='expediat') { ?>
<form action="index.php" method="post"><input type=hidden name=order_id value="<?=$product['order_id']?>"><button class="btn btn-danger">Confirma Expediere</button></form>
<?php }
